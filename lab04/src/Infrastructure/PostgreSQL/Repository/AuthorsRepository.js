...
const getByIdAndNamesAsync = async (id) => {
    console.info(`Getting the books names for authors with id ${id} from database async...`);

    const names_books = await queryAsync('SELECT books.id, books.name, books.author_id, authors.first_name, authors.last_name FROM books JOIN authors ON books.author_id = authors.id WHERE books.id = $1', [id]);
    return names_books[0];
};
